...
const { getUserById, companyForUser } = require('./store');
...
function completeAuthorization({ form, sessionUser }) {
    ...
    if (!sessionUser) {
        ...
        for (const key of ['client_id', 'redirect_uri', 'response_type', 'scope', 'state', 'code_challenge', 'code_challenge_method']) {
            if (typeof form[key] === 'string' && form[key]) params.set(key, form[key]);
        }
...
function loadLiveUser(userId) {
    ...
    return {
        ...user,
        companyRecordId: company?.id || user.companyRecordId || '',
