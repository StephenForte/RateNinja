...
/**
 * Drop expired settled entries. Pending loads are kept so in-flight requests
 * can still complete and (if still current) populate the cache.
 * @param {number} [now]
 */
...
async function cachedLoad(key, loader, ttlMs = config.cacheTtlMs) {
    ...
    pending = loader()
        .then(value => {
            // Skip re-cache if this load was invalidated (or superseded) while in flight.
            if (cache.get(key)?.pending === pending) setCached(key, value, ttlMs);
            return value;
        })
        .catch(error => {
...
function getAllRates({ force = false } = {}) {
    if (force) invalidateCache('rates:all');
...
async function fetchCompanyByRecordId(recordId) {
    const records = await fetchAllRecords(config.tables.companies, {
        filterByFormula: `RECORD_ID() = "${escapeFormulaString(recordId)}"`,
